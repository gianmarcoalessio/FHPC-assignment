...
echo "threads_per_socket, static_mean" > $datafile
...
do
	...
	# mpirun -np $sockets --map-by socket main.x -r -f "playground.pgm" -e 0 -n 5 -s 0 -k $size
	mpirun -np ${socket} --map-by node --bind-to socket main.x -r -f "playground.pgm" -e 1 -n 30 -s 0 -k $size
