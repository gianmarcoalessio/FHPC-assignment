...
int main(int argc, char **argv)
{
    ...
    */
    ...
    if (action == RUN)
    {
        ...
        int processed_bytes = processed_chunck * k;
        ...
        {
            unsigned char *playground = (unsigned char *)malloc(processed_bytes * sizeof(unsigned char));

            parallel_read_pgm_image((void **)&playground, fname, my_offset, processed_bytes);
            ...
            if (s >= 0)
            {
                ...
                if (e == ORDERED)
                {
                    ordered_evolution(playground, k, processed_chunck, my_offset, n, s > 0 ? s : n);
                }
                else
                {
                    static_evolution(playground, k, processed_chunck, my_offset, n, s > 0 ? s : n);
                }
